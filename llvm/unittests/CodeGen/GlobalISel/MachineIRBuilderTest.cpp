...
TEST_F(AArch64GISelMITest, TestBuildConstantFConstantDeath) {
  ...
  EXPECT_DEATH(B.buildConstant(LLT::float16(), APV32),
               "creating constant with the wrong size");
  EXPECT_DEATH(B.buildConstant(LLT::fixed_vector(2, LLT::float16()), APV32),
               "creating constant with the wrong size");

  // Test ConstantInt version breaks
  ConstantInt *CI = ConstantInt::get(Ctx, APV32);
  EXPECT_DEATH(B.buildConstant(LLT::float16(), *CI),
               "creating constant with the wrong size");
  EXPECT_DEATH(B.buildConstant(LLT::fixed_vector(2, LLT::float16()), *CI),
               "creating constant with the wrong size");
